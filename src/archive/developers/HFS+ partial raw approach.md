...
1.  The raw target image file itself is created..
    `dd if=/dev/zero of="$TARGET" bs=512k count=$(expr $SIZE '*' 2)`
    ..and attached to a vndevice node (does not behave as a disk device node)
    `/usr/libexec/vndevice attach /dev/vn0 "$TARGET"`
    Then the image/device is initialized and populated with a minimal partition map.
...
3.  The HFS partition image comes also originally from /dev/zero via `dd'.
    dd if=/dev/zero of="$T_TARGET_P3" bs=1024k count=600
    ...
    Finally, the HFS image is also deployed to the target image
    echo "ddying $T_TARGET_P2 to $RDEVICE"
    dd if="$T_TARGET_P3" of=$RDEVICE seek=$sectors bs=2048
...
![](/archive/img/developers/hfsplus/hfs-partial-raw-approach/mount%20puredarwin%20iso%20from%20puredarwin.png)
...
![](/archive/img/developers/hfsplus/hfs-partial-raw-approach/HFS%20and%20disk%20utility%20repair.png)
...
![](/archive/img/developers/hfsplus/hfs-partial-raw-approach/HFS%20bad%20volume%20header%20kernel%20panic.png)
...
![](/archive/img/developers/hfsplus/hfs-partial-raw-approach/HFS%20diff%20part3.png)
