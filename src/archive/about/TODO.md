...
With the X window system up and running, PureDarwin becomes a very compatible BSD clone, meaning that it should run any of the wide range of software available for that platform. As an example, most of the [pure](/archive/developers/Using%20MacPorts/Purity.md) binaries compiled through [the MacPorts project](/archive/developers/Using%20MacPorts/Using-MacPorts.md) can easily be copied into and used in a PureDarwin release. Let us know what you got running.
...
The basic Darwin system as provided by Apple is made up of over 400 different packages. Many of these have been included in releases of PureDarwin ([Xmas](/archive/users/Xmas.md) and [Nano](/archive/users/PureDarwin-Nano.md)). Many more are available from http://src.macosforge.org. **And they all need testing.** We know this method is empiric but we simply need to know what works, and (more importantly) what doesn't, so that we can focus our efforts there.
...
__Notes:__ See the [Network](/archive/developers/Network.md) page, the [KEXTs](/archive/developers/KEXTs/kexts.md) page, the [Graphics](/archive/developers/Graphics.md) page, the [Binary drivers](BinaryDrivers.md) page, the [ZFS](/archive/developers/ZFS.md) page, etc...
